...
as well as the block for each depth level (DownConvBlockBlock and UpConvBlockBlock).
...
class DilationBlock(nn.Module):
    """
    This block concatenates the results of 3 conv-norm-activ blocks using different dilation values.
    """
    def __init__(self, in_size, out_size, kernel_size=3, stride=1, padding=1, 
                 dilation_vals=(1, 3, 5), norm="instance", activ ="relu"):
        """
        Parameters
        ----------
        in_size: int or tuple of ints 
        ...
        padding: int or tuple of ints 
                number of zero-value pixels added at the image borders 
                (allows for instance to maintain the image dimensions)
        dilation_vals: tuple of ints
                defines the dilation values for each of the three dilated convultions
        norm: str
                defines the normalization layer type (Instance Norm or Batch Norm)
        activ: str
                defines the activation layer type (ReLu)
        ...
        # define the three conv-norm-activ blocks with different dilation values
        self.dilation1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size, stride,
                                                 padding=dilation_vals[0], dilation=dilation_vals[0]),
                                       norm_layer,
                                       activ_layer)
        self.dilation2 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size, stride,
                                                 padding=dilation_vals[1], dilation=dilation_vals[1]),
                                       norm_layer,
                                       activ_layer)
        self.dilation3 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size, stride,
                                                 padding=dilation_vals[2], dilation=dilation_vals[2]),
                                       norm_layer,
                                       activ_layer)
        self.conv = ConvNormActiv(out_size*3, out_size, kernel_size, stride, padding, norm)

    def forward(self, x):
        """
        Forward pass of the tensor.
        Performs the convulations at the given dilation values, concatenates the results along the channel dimension,
        then applies another conv-norm-activ block.
        """
        dil_1 = self.dilation1(x)
        dil_2 = self.dilation2(x)
        dil_3 = self.dilation3(x)
        x = torch.cat((dil_1, dil_2, dil_3), dim=1)
        x = self.conv(x)
        return x
...
class DownConvBlock(nn.Module):
    """
    Image processing block sequencing a ConvNormActiv, a Dilation block followed by a Pooling operation.
    Inherits from the torch.nn.Module class.
    """
    def __init__(self, in_size, out_size, kernel_size=3, stride=1, padding=1,
                 pooling=None, dilation_vals=(1, 3, 5), norm="instance", activ="relu"):
        """
        Parameters
        ----------
        in_size: int or tuple of ints 
        ...
        # combine a ConvNormActiv and a Dilation block
        self.conv = nn.Sequential(ConvNormActiv(in_size,  out_size, kernel_size, stride, 
                                                padding, norm, activ),
                                  DilationBlock(out_size, out_size, kernel_size, stride, 
                                                padding, dilation_vals, norm, activ))
...
class UNet_2D_Dilated(BaseNetwork):
    """
    ...
    def __init__(self, num_classes, in_channels=1, depth=4, 
                 start_channels=32, kernel_size=3, dilation_vals=(1, 3, 5),
        ...
        up_mode: str
                defines the upconvolution layer type
        pooling: str
                defines the pooling operation after the convolution blocknorm: str
                defines the normalization layer type (Instance Norm or Batch Norm)
        activ: str
                defines the activation layer type (ReLu)
        init_type: str
...
if __name__ == "__main__":
    net = UNet_2D_Dilated(num_classes=5)
